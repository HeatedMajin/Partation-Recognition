...
colors =['white','white','aqua', 'black', 'blue', 'brown', 'darkcyan', 'darkgreen', 'darkmagenta',
 'darkorchid', 'darkred', 'darkslategray', 'darkviolet', 'deeppink', 'fuchsia',
 'indigo', 'lime', 'magenta', 'maroon', 'navy', 'orangered']
def plotData(data,color,style):
    ...
    if data.shape[1] == 3:
        aa = (data[:,2]).reshape((1,788))
        aa = aa[0]
        bb = [colors[int(x+2)] for x in aa]
        plt.scatter(data[:,0], data[:,1],c=bb,marker=style)
    else:
